...
- **Response markers** — non-`none` modes ask the model to end its response with `(ephew-c)` / `(ephew-p)` / `(ephew-v)` / `(ephew-t)` (preceded by a space) so the user can see at a glance which mode handled the response.
