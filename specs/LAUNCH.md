...
- Name: `etherops/homebrew-ephew` (the `homebrew-` prefix is mandatory; brew strips it when computing the tap name).
- Visibility: public.
...
Uses `Language::Python::Virtualenv` to install ephew + every Python dependency into an isolated virtualenv under `Cellar/ephew/0.1.0/libexec`, then symlinks the `ephew` script into `Cellar/ephew/0.1.0/bin`. Sketch:
...
class Ephew < Formula
  ...
  # One `resource` block per transitive Python dependency.
  # Generated via `homebrew-pypi-poet`; see "Resource list" below.
  resource "fastapi" do
    url "https://files.pythonhosted.org/.../fastapi-0.111.0.tar.gz"
    sha256 "..."
  end
  # ... rumps, pyobjc-core, pyobjc-framework-Cocoa, uvicorn, httpx,
  #     httpcore, anyio, sniffio, h11, idna, certifi, starlette,
  ...
  test do
    assert_match "ephew 0.1.0", shell_output("#{bin}/ephew --version")
  end
end
```

**3. Generate the resource list.**

Brew formulas need every transitive Python dep listed explicitly with a pinned URL + SHA256. Two paths:

- **Initial generation:** `pip install homebrew-pypi-poet && pip install ephew && poet ephew`. Outputs the full resource block list, ready to paste into the formula.
- **On every release:** re-run `poet` against the new ephew install, paste the updated blocks. Or use `brew update-python-resources Formula/ephew.rb` which patches existing blocks in place.
...
3. In `etherops/homebrew-ephew`: paste the SHA256 into `Formula/ephew.rb`.
4. Test (step 4 above).
...
- `etherops/homebrew-ephew` repo exists (public, MIT) with `Formula/ephew.rb` and a small README.
- `brew tap etherops/ephew && brew install ephew` succeeds on a fresh Mac (or a clean test account on the dev Mac).
